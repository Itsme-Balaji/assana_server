...
const PelvicFloorWhyAssana = require("../../models/PelvicFloorModel/PelvicFloorWhyAssana");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await PelvicFloorWhyAssana.findOne();

      const PelvicFloorWhyAssanaData = { ...req.body };
      ...
      // Save or update
      const updatedDoc = await PelvicFloorWhyAssana.findOneAndUpdate(
        {},
        { $set: PelvicFloorWhyAssanaData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await PelvicFloorWhyAssana.findOne().select(
