...
const pelvicFloorPageModel = require("../../models/PelvicFloorModel/PelvicFloorPageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await pelvicFloorPageModel.findOne();

      const pelvicFloorPageData = { ...req.body };
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await pelvicFloorPageModel.findOne().select(
