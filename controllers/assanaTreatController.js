...
const assanaTreatModel = require("../models/assanaTreat");
...
router.post(
  "/dataAdd",
  upload.fields([
    { name: "TreatmentImg1", maxCount: 1 },
    { name: "TreatmentImg2", maxCount: 1 },
    { name: "TreatmentImg3", maxCount: 1 },
    { name: "TreatmentImg4", maxCount: 1 },
    ...
    try {
      ...
      for (const field of imageFields) {
        if (files[field] && files[field][0]) {
          ...
          assanaTreatData[field] = await uploadToSupabase(
            files[field][0],
            field
          );
        } else if (body[`${field}Old`]) {
          // Keep old Supabase URL if passed
          assanaTreatData[field] = body[`${field}Old`];
        }
      }

      // Save or update (only one doc allowed)
      const updatedDoc = await assanaTreatModel.findOneAndUpdate(
        {},
        { $set: assanaTreatData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await assanaTreatModel
      .findOne()
