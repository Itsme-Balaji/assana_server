...
const pilesorHaemorrhoidsPageModel = require("../../models/PilesorHaemorrhoidsModels/PilesorHaemorrhoidsPageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await pilesorHaemorrhoidsPageModel.findOne();

      const pilesorHaemorrhoidsPageData = { ...req.body };
      ...
      pilesorHaemorrhoidsPageData.needToKnowImg1 = await uploadToSupabase(
        req.files?.needToKnowImg1?.[0],
        req.body.needToKnowImg1Old || existingDoc?.needToKnowImg1
      );

      pilesorHaemorrhoidsPageData.needToKnowImg2 = await uploadToSupabase(
        req.files?.needToKnowImg2?.[0],
        req.body.needToKnowImg2Old || existingDoc?.needToKnowImg2
      );

      pilesorHaemorrhoidsPageData.needToKnowImg3 = await uploadToSupabase(
        req.files?.needToKnowImg3?.[0],
        req.body.needToKnowImg3Old || existingDoc?.needToKnowImg3
      );

      pilesorHaemorrhoidsPageData.needToKnowImg4 = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await pilesorHaemorrhoidsPageModel.findOneAndUpdate(
        {},
        { $set: pilesorHaemorrhoidsPageData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await pilesorHaemorrhoidsPageModel.findOne().select(
