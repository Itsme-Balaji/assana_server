...
const afterAnalSurgeryPageModel = require("../../models/AfterAnalSurgery/AfterAnalSurgeryPageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await afterAnalSurgeryPageModel.findOne();

      const afterAnalSurgeryPageData = { ...req.body };
      ...
      afterAnalSurgeryPageData.needToKnowImg = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await afterAnalSurgeryPageModel.findOneAndUpdate(
        {},
        { $set: afterAnalSurgeryPageData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await afterAnalSurgeryPageModel.findOne().select(
