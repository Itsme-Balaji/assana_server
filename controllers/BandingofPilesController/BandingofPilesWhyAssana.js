...
const BandingofPilesWhyAssana = require("../../models/BandingofPiles/BandingofPilesWhyAssana");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await BandingofPilesWhyAssana.findOne();

      const BandingofPilesWhyAssanaData = { ...req.body };
      ...
      // Handle files
      BandingofPilesWhyAssanaData.Why_Assana_Image1 = await uploadToSupabase(
        req.files?.Why_Assana_Image1?.[0],
        req.body.Why_Assana_Image1Old || existingDoc?.Why_Assana_Image1
      );

      BandingofPilesWhyAssanaData.Why_Assana_Image2 = await uploadToSupabase(
        req.files?.Why_Assana_Image2?.[0],
        req.body.Why_Assana_Image2Old || existingDoc?.Why_Assana_Image2
      );

      BandingofPilesWhyAssanaData.Why_Assana_Image3 = await uploadToSupabase(
        req.files?.Why_Assana_Image3?.[0],
        req.body.Why_Assana_Image3Old || existingDoc?.Why_Assana_Image3
      );

      BandingofPilesWhyAssanaData.Why_Assana_Image4 = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await BandingofPilesWhyAssana.findOneAndUpdate(
        {},
        { $set: BandingofPilesWhyAssanaData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await BandingofPilesWhyAssana.findOne().select(
