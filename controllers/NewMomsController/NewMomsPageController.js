...
const newMomsPageModel = require("../../models/NewMomsModel/NewMomsPageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await newMomsPageModel.findOne();

      const newMomsPageData = { ...req.body };
      ...
      newMomsPageData.needToKnowImg = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await newMomsPageModel.findOneAndUpdate(
        {},
        { $set: newMomsPageData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await newMomsPageModel.findOne().select(
