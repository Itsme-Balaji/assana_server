...
const laserSurgeryPageModel = require("../../models/LaserSurgeryModel/LaserSurgeryPageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await laserSurgeryPageModel.findOne();

      const laserSurgeryPageData = { ...req.body };
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await laserSurgeryPageModel.findOne().select(
