...
const LaserSurgeryProblems = require("../../models/LaserSurgeryModel/LaserSurgeryProblems");
...
router.post(
  "/dataAdd",
  upload.fields([
    { name: "LaserSurgery_Image", maxCount: 1 },


  ]),
  async (req, res) => {
    try {
      let existingDoc = await LaserSurgeryProblems.findOne();

      const LaserSurgeryProblemsData = { ...req.body };
      ...
      // Save or update
      const updatedDoc = await LaserSurgeryProblems.findOneAndUpdate(
        {},
        { $set: LaserSurgeryProblemsData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await LaserSurgeryProblems.findOne().select(
