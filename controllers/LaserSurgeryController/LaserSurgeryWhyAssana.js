...
const LaserSurgeryWhyAssana = require("../../models/LaserSurgeryModel/LaserSurgeryWhyAssana");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await LaserSurgeryWhyAssana.findOne();

      const LaserSurgeryWhyAssanaData = { ...req.body };
      ...
      // Handle files
      LaserSurgeryWhyAssanaData.Why_Assana_Image1 = await uploadToSupabase(
        req.files?.Why_Assana_Image1?.[0],
        req.body.Why_Assana_Image1Old || existingDoc?.Why_Assana_Image1
      );

      LaserSurgeryWhyAssanaData.Why_Assana_Image2 = await uploadToSupabase(
        req.files?.Why_Assana_Image2?.[0],
        req.body.Why_Assana_Image2Old || existingDoc?.Why_Assana_Image2
      );

      LaserSurgeryWhyAssanaData.Why_Assana_Image3 = await uploadToSupabase(
        req.files?.Why_Assana_Image3?.[0],
        req.body.Why_Assana_Image3Old || existingDoc?.Why_Assana_Image3
      );

      LaserSurgeryWhyAssanaData.Why_Assana_Image4 = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await LaserSurgeryWhyAssana.findOneAndUpdate(
        {},
        { $set: LaserSurgeryWhyAssanaData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await LaserSurgeryWhyAssana.findOne().select(
