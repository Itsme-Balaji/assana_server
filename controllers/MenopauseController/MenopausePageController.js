...
const menopausePageModel = require("../../models/MenopauseModel/MenopausePageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await menopausePageModel.findOne();

      const menopausePageData = { ...req.body };
      ...
      // Handle files
      menopausePageData.colorectalBannerImg = await uploadToSupabase(
        req.files?.colorectalBannerImg?.[0],
        req.body.colorectalBannerImgOld || existingDoc?.colorectalBannerImg
      );

      menopausePageData.needToKnowImg = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await menopausePageModel.findOneAndUpdate(
        {},
        { $set: menopausePageData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await menopausePageModel.findOne().select(
