...
const ColorectalSymptomsWhyAssana = require("../../models/ColorectalSymptomsModel/ColorectalSymptomsWhyAssana");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await ColorectalSymptomsWhyAssana.findOne();

      const ColorectalSymptomsWhyAssanaData = { ...req.body };
      ...
      // Handle files
      ColorectalSymptomsWhyAssanaData.Why_Assana_Image1 = await uploadToSupabase(
        req.files?.Why_Assana_Image1?.[0],
        req.body.Why_Assana_Image1Old || existingDoc?.Why_Assana_Image1
      );

      ColorectalSymptomsWhyAssanaData.Why_Assana_Image2 = await uploadToSupabase(
        req.files?.Why_Assana_Image2?.[0],
        req.body.Why_Assana_Image2Old || existingDoc?.Why_Assana_Image2
      );

      ColorectalSymptomsWhyAssanaData.Why_Assana_Image3 = await uploadToSupabase(
        req.files?.Why_Assana_Image3?.[0],
        req.body.Why_Assana_Image3Old || existingDoc?.Why_Assana_Image3
      );

      ColorectalSymptomsWhyAssanaData.Why_Assana_Image4 = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await ColorectalSymptomsWhyAssana.findOneAndUpdate(
        {},
        { $set: ColorectalSymptomsWhyAssanaData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await ColorectalSymptomsWhyAssana.findOne().select(
