...
const analFissurePageModel = require("../../models/AnalFissureModel/AnalFissurePageModel");
...
router.post(
  ...
  async (req, res) => {
    try {
      let existingDoc = await analFissurePageModel.findOne();

      const analFissurePageData = { ...req.body };
      ...
      analFissurePageData.needToKnowImg = await uploadToSupabase(
      ...
      // Save or update
      const updatedDoc = await analFissurePageModel.findOneAndUpdate(
        {},
        { $set: analFissurePageData },
...
router.get("/dataGet", async (req, res) => {
  try {
    ...
    const data = await analFissurePageModel.findOne().select(
